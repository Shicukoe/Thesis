# hard_erp — clean prompt history (RQ1/RQ2 input, feed one prompt at a time)

Same 45-prompt history used for the basic / loose / strict candidates, so spec-guided
is measured on the identical input. No injected conflicts — this is the RQ1/RQ2
faithfulness setting; the ordinary edit chains are left intact (e.g. due date set to
30 days at prompt 17 then changed to 15 at prompt 18 — a resolved change, latest wins).
The four planted semantic conflicts live only in the separate RQ3 file
(../conflict/hard_erp_conflict_history.md); do NOT add them here.
